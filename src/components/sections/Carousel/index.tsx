...
import { FLAVORS, SPINS_ON_CHANGE } from "./constants";
...
export const Carousel = () => {
  ...
  const changeFlavor = (index: number) => {
    ...
    const spinDirection = index > currentFlavorIndex ? "-=" : "+=";
    const spinAmount = `${spinDirection}${Math.PI * 2 * SPINS_ON_CHANGE}`;
        ...
        <View className="aspect-square h-[70vmin] min-h-40">
          <Center position={[0, 0, 0.5]}>
            <FloatingCan
              ...
              floatIntensity={0.3}
              rotationIntensity={1}
              sodaCan={{ flavor: FLAVORS[currentFlavorIndex].flavor }}
            />
          </Center>
